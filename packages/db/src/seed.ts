...
import { db, skills as skillsTable, skillCategories } from "./index";
...
const readSkills = async () => {
  try {
    ...
    const raw = await readFile("./data/skills.json", { encoding: "utf8" });
...
const readCategories = async () => {
  try {
    const raw = await readFile("./data/category.json", { encoding: "utf8" });
